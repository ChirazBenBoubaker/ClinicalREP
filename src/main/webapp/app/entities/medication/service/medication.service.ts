...
type RestOf<T extends IMedication | NewMedication> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class MedicationService {
  ...
  protected convertDateFromClient<T extends IMedication | NewMedication | PartialUpdateMedication>(medication: T): RestOf<T> {
    return {
      ...medication,
      date: medication.date?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMedication: RestMedication): IMedication {
    return {
      ...restMedication,
      date: restMedication.date ? dayjs(restMedication.date) : undefined,
    };
  }
