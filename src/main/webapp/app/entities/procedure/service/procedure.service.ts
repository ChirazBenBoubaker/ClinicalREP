...
type RestOf<T extends IProcedure | NewProcedure> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class ProcedureService {
  ...
  protected convertDateFromClient<T extends IProcedure | NewProcedure | PartialUpdateProcedure>(procedure: T): RestOf<T> {
    return {
      ...procedure,
      date: procedure.date?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProcedure: RestProcedure): IProcedure {
    return {
      ...restProcedure,
      date: restProcedure.date ? dayjs(restProcedure.date) : undefined,
    };
  }
