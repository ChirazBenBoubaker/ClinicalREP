...
type RestOf<T extends IObservation | NewObservation> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class ObservationService {
  ...
  protected convertDateFromClient<T extends IObservation | NewObservation | PartialUpdateObservation>(observation: T): RestOf<T> {
    return {
      ...observation,
      date: observation.date?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restObservation: RestObservation): IObservation {
    return {
      ...restObservation,
      date: restObservation.date ? dayjs(restObservation.date) : undefined,
    };
  }
